...
public class ChangeJarTest {
    ...
    // A String with a decimal, but no numbers after it
    @Test (expected = IllegalArgumentException.class)
    public void testStringConstructorNoDecimalPlace() {
        ChangeJar jar = new ChangeJar("500.");
    }
    ...
    public void testToString() {
        ChangeJar jar = new ChangeJar(1,1,1,1);
        String s = jar.toString();

        assertTrue(s.contains("Quarter"));
        assertTrue(s.contains("Dime"));
        assertTrue(s.contains("Penny"));
        assertTrue(s.contains("Nickel"));
        assertFalse(s.contains("s"));

        ChangeJar jar2 = new ChangeJar(2,2,2,2);
        String s2 = jar2.toString();

        assertTrue(s2.contains("Quarters"));
        assertTrue(s2.contains("Dimes"));
        assertTrue(s2.contains("Pennies"));
        assertTrue(s2.contains("Nickels"));
    }
    ...
    // testing adding a ChangeJar to another ChangeJar
    @Test
    public void testAddChangeJar() {
        for(int i = 0; i < 100; i++) {
            ChangeJar jar = new ChangeJar();
            ChangeJar jar2 = new ChangeJar(i,0,0,0);

            jar.add(jar2);

            assertTrue(i == jar.getQuarters());
        }

        for(int i = 0; i < 100; i++) {
            ChangeJar jar = new ChangeJar();
            ChangeJar jar2 = new ChangeJar(0,i,0,0);

            jar.add(jar2);

            assertTrue(i == jar.getDimes());
        }

        for(int i = 0; i < 100; i++) {
            ChangeJar jar = new ChangeJar();
            ChangeJar jar2 = new ChangeJar(0,0,i,0);

            jar.add(jar2);

            assertTrue(i == jar.getNickels());
        }

        for(int i = 0; i < 100; i++) {
            ChangeJar jar = new ChangeJar();
            ChangeJar jar2 = new ChangeJar(0,0,0,i);

            jar.add(jar2);

            assertTrue(i == jar.getPennies());
        }
    }
