...
public class MySingleWithOutTailLinkedListTest {
    ...
    public void size() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (randomPick == 0) {
                randomCamp = new TentOnly("T" + i, checkin, checkout, null, 10);
            }
            if (randomPick == 1) {
                randomCamp = new RV("RV" + i, checkin, checkout, null, 1000);
            }
            list.add(randomCamp);
    ...
    public void remove() {
        ...
        TentOnly tent = new TentOnly("T" + rand.nextInt(100),checkin, checkout, null, rand.nextInt(100));
        RV rv = new RV("RV" + rand.nextInt(100), checkin, checkout, null, rand.nextInt(10000));
        list.add(tent);
        list.add(rv);
        ...
        for (int i = 0; i < 100; i++) {
            if (i % 2 == 0) {
                removed = new TentOnly("T" + rand.nextInt(100),checkin, checkout, null, rand.nextInt(100));
            }
            else {
                removed = new RV("RV" + rand.nextInt(100), checkin, checkout, null, rand.nextInt(10000));
            }
            list.add(removed);
        }
        ...
        // FIX ME: Add method creates a list that has tents after RVs
        // Remove test does not properly function as a result.
